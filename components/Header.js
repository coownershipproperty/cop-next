...
// The uppercase code (ENG, ES, FR ...) is the visible label, rendered at
// the same size as the main drawer nav rows; the native language name
// stays in the markup for screen readers only (CSS clips it).
function LocaleTag({ loc }) {
  return (
    <>
      <span className="cop-lang-code">{loc === 'en' ? 'ENG' : loc.toUpperCase()}</span>
