  ...
  return (
    <header className="cop-header scrolled" id="cop-header">
      <div className="cop-logo">
        <a href="/"><img src="/wp-content/uploads/MAIN-LOGO-COP.svg" alt="Co-Ownership Property" /></a>
      </div>
      <nav className="cop-nav" id="cop-nav">
        {navLinks.map(({ href, label, extra }) => {
          const isActive = path === href || path.startsWith(href + '/') || (href !== '/' && path.startsWith(href));
