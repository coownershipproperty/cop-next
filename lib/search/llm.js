...
function adaptBody(body, errText) {
  ...
  // set reasoning_effort to 'none'." Learned in production on 5 Aug.
  if (e.includes('reasoning_effort') && next.reasoning_effort !== 'none') {
...
export async function* chatStream(messages, opts = {}) {
  ...
  let body = {
    ...
    stream: true,
    // Luna rejects function tools unless reasoning is off — and 'none' is
    // also its fastest mode, which is exactly what a conversation wants.
