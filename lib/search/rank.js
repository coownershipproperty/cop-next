...
/**
 * Buyers say "south of France"; the corpus says country France. A phrase that
 * matches no home directly is searched for a known place name INSIDE it — the
 * longest one wins, so "the Spanish costas" prefers a region over "spain" if
 * both ever appear. A phrase containing no known place ("Japan") is kept
 * verbatim, because failing to match anything is exactly how the honest
 * we-do-not-cover-that path gets triggered.
 */
export function resolvePlaces(places, props) {
  ...
  return places.map(q => {
    const nq = norm(q);
    if (props.some(p => placeText(p).includes(nq))) return q;
    let best = null;
    for (const term of vocab) {
      if (term.length >= 4 && nq.includes(term) && (!best || term.length > best.length)) best = term;
    }
    return best || q;
  });
}
