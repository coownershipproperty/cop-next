...
export function applyFilters(props, filters, minResults = 4) {
  ...
  const test = (p, relaxed) => {
    ...
    // stance is not treated as a no.
    if (filters.needs_rental && !relaxed.has('rental') && rentalStance(p.partner) === 'forbidden') return false;
  ...
  const STEPS = [
    ...
    { key: 'rental', label: 'included homes where the rental rules are unconfirmed',
      live: f => f.needs_rental === true },
