...
FILTERS are hard constraints. Only set one if the buyer genuinely stated it.
  ...
  places                free text location names exactly as the buyer said them: ["Mallorca"], ["Spain"], ["French Alps"].
...
If the buyer names a place not in the collection, still put it in places — the search handles a near miss honestly and says so.
...
{
  "summary": "...",
  "filters": { "beds_min": null, "beds_max": null, "budget_max": null, "budget_currency": "EUR", "places": [], "exclude_places": [], "property_types": [], "needs_rental": false },
  "weights": { "facet": 2 },
...
FOR EACH HOME write "why", 15 to 30 words, one or two sentences. It must reference at least one concrete piece of the supplied evidence. If the only evidence is a listing feature such as a pool, say that plainly rather than padding.
...
If the search was widened to find results, mention that in the headline.

Reply with exactly:
{ "headline": "...", "why": { "property-slug": "...", "property-slug-2": "..." } }`;
