...
export function intentSystemPrompt(vocab = {}) {
  ...
  // The full city list costs a few hundred tokens and buys real geographic
  // intelligence: the model knows where Antibes is, so "south of France",
  // "the Riviera" or "near the Italian lakes" can be answered with the
  // specific listed places that genuinely lie there, instead of a whole
  // country with Paris in it.
...
FILTERS are hard constraints. Only set one if the buyer genuinely stated it.
  ...
  places                location names, chosen from the collection's own lists below. Use your geography: pick the MOST SPECIFIC listed places that genuinely lie inside the area the buyer described. "south of France" -> the listed cities/regions actually in the south of France (never Paris). "Balearics" -> the listed Balearic islands' places. "the Italian lakes" -> the listed lake towns. Use several names when several fit. Use the whole country only when the buyer meant the whole country. Keep the buyer's exact words only when no listed place applies at all.
...
export function fallbackIntent(query, vocab = {}) {
  ...
  //
  // A directional qualifier right before the place travels WITH it — "south
  // of france" goes into places whole, not as bare "France" — because the
  // resolver in rank.js turns "south of X" into the actual southern homes,
  // and it can only do that if the direction survives extraction.
  const places = [];
  ...
  for (const name of [...(vocab.cities || []), ...(vocab.regions || []), ...(vocab.countries || [])]) {
    ...
    if (at === -1) continue;
    const before = hay.slice(Math.max(0, at - 24), at);
    const dir = /\b(south|north|east|west|southern|northern|eastern|western)(?:ern)?(?:\s+(?:of|coast(?:\s+of)?|part(?:\s+of)?))?\s*$/.exec(before);
    const phrase = dir ? `${dir[1]} of ${name}` : name;
    if (!places.some(p => p.toLowerCase().includes(n))) places.push(phrase);
  }
