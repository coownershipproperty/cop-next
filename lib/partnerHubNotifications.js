...
const ADMIN_EMAIL = process.env.PARTNER_HUB_ADMIN_EMAIL || 'dylan@co-ownership-property.com'; // internal notices → Dylan's inbox (David, 12 Sep 2026)
...
async function deliver({ db, lead, eventType, recipient, cc = [], subject, html }) {
  ...
  try {
    ...
    const delivery = await sendHtml({ to: recipient, cc: cc.length ? cc : undefined, subject, html, noLeadCopy: true });
...
export async function notifyPartnerOfLead({ db, partner, lead, eventType = 'lead_assigned', note }) {
  ...
  const reviewPrefix = partner.test_routing ? '[REVIEW] ' : '';
  ...
  return deliver({
    ...
    subject: eventType === 'test'
      ? `[REVIEW] ${partner.display_name} Partner Hub routing`
      : eventType === 'partner_note'
        ? `${reviewPrefix}COP note on ${leadName}`
        : `${reviewPrefix}New COP lead — ${leadName}`,
    html: emailShell({
      label: partner.test_routing ? 'INTERNAL REVIEW' : 'PRIVATE PARTNER NOTIFICATION',
      heading: eventType === 'test'
        ? 'Partner Hub routing check'
        : eventType === 'partner_note' ? 'COP added a note to your lead' : 'A lead is ready in your workspace',
      body: eventType === 'test'
...
export async function notifyAdminOfHelpRequest({ db, partner, lead, note }) {
  const leadName = `${lead.first_name} ${lead.last_name}`.trim();
  return deliver({
    db,
    lead,
    eventType: 'partner_help_request',
    recipient: ADMIN_EMAIL,
    subject: `Help requested by ${partner.display_name} — ${leadName}`,
    html: emailShell({
      label: 'PARTNER REQUESTED COP HELP',
      heading: `${partner.display_name} would like support`,
      body: `<p><strong>Lead:</strong> ${escapeHtml(leadName)}</p><p><strong>Request:</strong> ${escapeHtml(note)}</p>`,
      buttonLabel: 'Open lead in COP Admin',
      buttonUrl: `${SITE_URL}/admin/partners/`,
    }),
  });
}
