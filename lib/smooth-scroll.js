...
export function initSmoothScroll({ ease = 0.11, wheelMultiplier = 1 } = {}) {
  ...
  function onWheel(e) {
    if (motionPreference.matches || e.ctrlKey || e.metaKey || e.defaultPrevented || Math.abs(e.deltaX) > Math.abs(e.deltaY)) return;
    ...
    target = Math.min(maxScroll(), Math.max(0, target + e.deltaY * unit * wheelMultiplier));
  }
  ...
  window.addEventListener('pointerdown', resync, { passive: true });
  window.addEventListener('keydown', resync);
  window.addEventListener('rd:navigation', resync);
  motionPreference.addEventListener('change', resync);
  return () => {
    stop();
    ...
    window.removeEventListener('rd:navigation', resync);
    motionPreference.removeEventListener('change', resync);
  };
}
