...
export async function queueEmail({
  ...
  // Always save to queue first — gives us a full audit trail. An auto-send
  // row starts as 'sending' (claimed) and only becomes 'sent' once Resend has
  // accepted it; it used to be written as 'sent' BEFORE the send, with the
  // send itself fire-and-forget, so a rejected email showed as sent and the
  // gallery dedupe then refused to retry it for 24 h (7 Sep 2026 audit).
  const initialStatus = (autoSend && !sendAfter) ? 'sending' : 'pending';
  const db = getDb();
  ...
  if (autoSend && !sendAfter) {
    ...
    // the queue processor does. Awaited: the caller learns about a rejected
    // send (it throws), and the row records what really happened.
    try {
      await sendHtml({
      ...
      if (data?.id) {
        const { error: uErr } = await db.from('email_queue')
          .update({ status: 'sent', sent_at: new Date().toISOString() }).eq('id', data.id);
        if (uErr) console.error('[EmailQueue] sent-status write failed:', uErr.message);
        else { data.status = 'sent'; data.sent_at = new Date().toISOString(); }
      }
    } catch (err) {
      console.error('[EmailQueue] auto-send failed:', err.message);
      if (data?.id) {
        await db.from('email_queue')
          .update({ status: 'error', notes: `${notes ? notes + ' — ' : ''}send failed: ${err.message}` }).eq('id', data.id);
      }
      throw err;
    }
  }
 ...
 */
/**
 * Every email a LEAD receives is also copied (BCC) to Dylan's Gmail, so one
 * inbox holds the whole conversation: the copy carries the same Message-ID,
 * Gmail threads the lead's reply underneath it, and the hourly drafter can
 * draft the next reply INTO that thread. Internal mail (team notifications,
 * task reminders, anything to a COP/Domosno address) is never copied, and the
 * newsletter batch path does not go through here. LEAD_COPY_BCC=off disables it.
 * (David, 8 Sep 2026: "it's not all connected".)
 */
export const LEAD_COPY_TO = process.env.LEAD_COPY_TO || 'dylan@co-ownership-property.com';
const INTERNAL_DOMAINS = ['co-ownership-property.com', 'domosno.com'];
export function leadCopyFor(to) {
  if (String(process.env.LEAD_COPY_BCC || '').toLowerCase() === 'off') return null;
  const list = (Array.isArray(to) ? to : [to]).map((a) => String(a || '').toLowerCase().trim());
  if (!list.length || list.some((a) => INTERNAL_DOMAINS.some((d) => a.endsWith('@' + d)))) return null;
  return LEAD_COPY_TO;
}

export async function sendHtml({ to, cc, subject, html, text, from, replyTo, headers, idempotencyKey, log, noLeadCopy }) {
  // A lead-facing direct send (`log` present) never goes to an address that
  // hard-bounced or filed a spam complaint, whatever form was just filled in.
  // Plain unsubscribes still receive transactional replies. Fails open on a
  // DB error — a reply to a real enquiry beats a missed suppression lookup.
  if (log) {
    try {
      const first = String(Array.isArray(to) ? to[0] : to || '').trim().toLowerCase();
      const { data: hard } = await getDb().from('suppressions').select('reason')
        .ilike('email', first).in('reason', ['bounced', 'complained']).limit(1);
      if (hard && hard.length) {
        throw Object.assign(new Error(`skipped — address is ${hard[0].reason}`), { skipped: true, suppressed: hard[0].reason });
      }
    } catch (e) {
      if (e.skipped) throw e;
      console.error('[sendHtml] suppression lookup failed (sending anyway):', e.message);
    }
  }
  const bcc = noLeadCopy ? null : leadCopyFor(to);
...
export async function sendHtmlBatch(messages) {
  ...
  const payload = messages.map((m) => ({
    ...
    ...(m.headers ? { headers: m.headers } : {}),   // per-message List-Unsubscribe etc.
    ...(m.text ? { text: m.text } : {}),
  }));
