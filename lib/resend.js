...
export async function queueEmail({
  ...
  // sendAfter always forces queue (can't auto-send a future email)
  if (autoSend && !sendAfter) {
    // Live mode — send immediately
    return sendHtml({ to, subject, html, from, replyTo });
  }

  // Draft mode — save to queue
  const db = getDb();
 ...
 */
async function sendHtml({ to, subject, html, from, replyTo }) {
