...
import { GUIDE_ORDER, GUIDE_HUB_COPY } from '@/lib/guides-hub-copy';
import { getInventory, fillInventoryDeep, inventorySummary } from '@/lib/inventory';

// guides-meta `country` → inventory key
const GUIDE_COUNTRY = { spain: 'spain', france: 'france', italy: 'italy', usa: 'usa' };
...
export async function buildGuideProps(locale, slug) {
  const meta = readGuideMeta(locale);
  ...
  const filled = fillInventoryDeep({ entry, body: injectH2Ids(fs.readFileSync(cp, 'utf-8')) }, inv, { locale });
  ...
  const inventory = inventorySummary(inv, { country: GUIDE_COUNTRY[slug], locale });
  return { props: { locale, slug, entry: filled.entry, body, faqs, wordCount, section: 'guide', altLocales, inventory }, revalidate: 3600 };
}

export async function buildGuideHubProps(locale) {
  const meta = readGuideMeta(locale);
  const inv = await getInventory();
  ...
  const items = ordered.map(slug => ({
    ...
    // Computed, never typed: the hub said 82 for Spain five days after it was 96.
    homesListed: (inv.countries[GUIDE_COUNTRY[slug]] || {}).available || null,
  })).map(it => fillInventoryDeep(it, inv, { locale }));
  const copy = fillInventoryDeep(GUIDE_HUB_COPY[locale], inv, { locale });
  ...
  return { props: { locale, items, altLocales, copy, inventory: inventorySummary(inv, { locale }) }, revalidate: 3600 };
}
