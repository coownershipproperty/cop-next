...
export function initReveals(router) {
      ...
      if (!io) {
        io = new IntersectionObserver((entries) => {
          for (const en of entries) {
            // In view, or already scrolled past (a fast inertial scroll can
            // jump a short block clean over the viewport in one frame).
  ...
  scan();
  // React updates (including Fast Refresh) can replace blocks or reset their
  // classes after the initial scan. Register those blocks again so rv-ready
  // cannot leave newly rendered content permanently invisible.
  const mutations = new MutationObserver(records => {
    const needsScan = records.some(record => record.type === 'childList'
      || (record.target.matches?.('[data-rv]') && !record.target.classList.contains('rv-in')));
    if (!needsScan || scanFrame !== null) return;
