 ...
 *   - More unlocks (same person) → ignored. One follow-up per person, full stop —
 *     this is what guarantees a multi-unlocker never gets multiple emails.
 *   - Contact submits enquiry → cancel the pending follow-up (best-effort) so the
 *     enquiry auto-reply isn't doubled up.
...
// Markup helpers live in lib/emailShell.js so a caller that only wants the
// shell does not have to construct a Resend client to get it. Re-exported
// here because half the codebase already imports them from this module.
import { escapeHtml, signatureHtml, emailShell } from '@/lib/emailShell';
export { escapeHtml, signatureHtml, emailShell };
...
const COPY = {
  ...
  de: {
    ...
    intro: (links) => `Danke, dass Sie sich für ${links} interessieren!`,
    offerSingle: "Ich bringe Sie gern mit dem Spezialistenteam zusammen, das dahintersteht — vorab: Haben Sie Fragen zur Immobilie oder zum Miteigentum, die ich für Sie weitergeben kann?",
    offerMulti:  "Ich bringe Sie gern mit den Spezialistenteams zusammen, die dahinterstehen — vorab: Haben Sie Fragen zu diesen Immobilien oder zum Miteigentum, die ich für Sie weitergeben kann?",
    close: "Sobald ich von Ihnen höre, stelle ich den Kontakt direkt her.",
    sign: "Dylan",
    and: " und ",
  ...
  it: {
    ...
    intro: (links) => `Grazie per il Suo interesse per ${links}!`,
    offerSingle: "Mi farebbe piacere metterLa in contatto con il team di specialisti che se ne occupa — prima di farlo, ha qualche domanda che posso girare loro sull'immobile o sul modello di comproprietà?",
    offerMulti:  "Mi farebbe piacere metterLa in contatto con i team di specialisti che se ne occupano — prima di farlo, ha qualche domanda che posso girare loro su queste case o sul modello di comproprietà?",
    close: "Appena mi risponde, faccio subito le presentazioni.",
    sign: "Dylan",
    and: " e ",
  ...
  nl: {
    ...
    greetingName:  (n) => `Hallo ${n},`,
    greetingNoName: "Hallo,",
    intro: (links) => `Bedankt voor uw interesse in ${links}!`,
    offerSingle: "Ik breng u graag in contact met het gespecialiseerde team achter deze woning — heeft u voordat ik dat doe nog vragen die ik aan hen kan doorgeven, over de woning of over mede-eigendom?",
    offerMulti:  "Ik breng u graag in contact met de gespecialiseerde teams achter deze woningen — heeft u voordat ik dat doe nog vragen die ik kan doorgeven, over deze woningen of over mede-eigendom?",
    close: "Zodra ik van u hoor, regel ik de introductie meteen.",
    sign: "Dylan",
    and: " en ",
  ...
  pt: {
    ...
    greetingNoName: "Olá,",
    intro: (links) => `Obrigado pelo seu interesse em conhecer ${links}!`,
    offerSingle: "Gostava muito de pôr a equipa especializada que está por trás desta casa em contacto consigo — antes de o fazer, tem alguma dúvida que eu lhes possa transmitir sobre a propriedade ou sobre o modelo de compropriedade?",
    offerMulti:  "Gostava muito de pôr as equipas especializadas que estão por trás delas em contacto consigo — antes de o fazer, tem alguma dúvida que eu lhes possa transmitir sobre estas casas ou sobre o modelo de compropriedade?",
    close: "Assim que tiver a sua resposta, faço a apresentação de imediato.",
    sign: "Dylan",
    and: " e ",
...
/** Per-property "photos & floor plans" links block (mirrors the live sensor
 *  variants in pages/api/process-gallery-followups.js and lib/email/templates.js). */
function galleryLinksHtml(properties, t) {
  ...
  const items = withLinks.map(p =>
    `<li style="margin:0 0 6px;">${escapeHtml(p.title)} — <a href="${escapeHtml(p.galleryUrl)}" style="color:#111111;text-decoration:underline;">${t.galleryLinkLabel}</a></li>`
  ).join('');
  return `
 ...
 * @param {{title:string,url:string|null,galleryUrl:string|null}[]} properties  — deduplicated, ≥1
...
function normProps(arr) {
  ...
  for (const p of arr || []) {
    ...
    seen.add(key);
    out.push({ title: p.title, url: p.url || null, galleryUrl: p.galleryUrl || null });
  }
  return out;
}
...
export async function scheduleOrUpdateGalleryFollowup({ contactId, email, firstName, locale, property }) {
  ...
  if (row) {
    ...
    // A follow-up already exists for this person inside the cooldown window —
    // scheduled, sent, or cancelled. Never schedule a second one. This single
    // rule is what guarantees ONE email no matter how many galleries they
    // unlock: the first unlock schedules it, every later unlock is ignored.
