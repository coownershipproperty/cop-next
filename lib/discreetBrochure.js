 ...
 * unlock". The email IS the listing; its only call to action is an enquiry
 * (prefilled mailto to Dylan). Never names the operator.
 */
import { render } from '@react-email/render';
import DiscreetBrochureEmail from '@/emails/discreet-brochure';
import { createSupabaseAdminClient } from '@/lib/supabaseAdmin';
import { sendHtml } from '@/lib/resend';
import { formatPrice, formatPriceRounded } from '@/lib/newsletter/personalize';
...
export async function buildDiscreetBrochureProps({ slug, firstName, email, locale = 'en' }) {
  ...
  const subjectLine = `Enquiry — ${[p.city, name].filter(Boolean).join(', ')}`;
  const bodyLine = `Hi Dylan,\n\nI've read the brochure for ${p.title || slug} and I'd like to know more.\n\n`;
  const enquiryUrl = `mailto:dylan@co-ownership-property.com?subject=${encodeURIComponent(subjectLine)}&body=${encodeURIComponent(bodyLine)}`;
  ...
  return {
    ...
    place,
    name,
    price:        formatPriceRounded(p.price, p.currency),
...
export async function sendDiscreetBrochure({ to, firstName, slug, locale, contactId, leadId, trackingPixelHtml }) {
  ...
  await sendHtml({
    ...
    log: {
      ...
      propertyTitle: _title || null, propertyUrl: `${BASE}/property/${slug}/`,
