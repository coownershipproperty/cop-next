 ...
 *  adding another one nobody remembers to set. */
function secret() {
  return process.env.SIGNIN_SECRET || process.env.UNSUB_SECRET || process.env.CRON_SECRET || '';
}

/** Every secret a presented token may have been signed with, preferred first. */
function acceptedSecrets() {
  const out = [];
  for (const s of [process.env.SIGNIN_SECRET, process.env.UNSUB_SECRET, process.env.CRON_SECRET]) {
