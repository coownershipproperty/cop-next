...
/**
 * Create or enrich the one active CRM lead linked to a contact.
 *
 * Property-specific enquiries remain available through the lead shortlist and
 * activity timeline instead of creating another dashboard card for the same
 * person. The database RPC holds a per-contact advisory lock so rapid repeated
 * submissions cannot race into duplicate active leads.
 */
export async function createLead({ contactId, propertySlug, propertyTitle, mainRegion, subregion, partner, message, budget, attribution, enquiryPageUrl }) {
  const db = getClient();
  const range = parseBudgetRange(budget);

  const { data, error } = await db
    .rpc('merge_or_create_contact_lead', {
      p_contact_id: contactId,
      p_property_slug: propertySlug || null,
      p_property_title: propertyTitle || null,
      p_main_region: mainRegion || null,
      p_subregion: subregion || null,
      // Callers that know the home's partner pass it (unlock-drive does); it
      // was always dropped here before (7 Sep 2026 audit).
      p_partner: partner || null,
      p_message: message || null,
      p_budget_min: range.min,
      p_budget_max: range.max,
      p_first_visit_at: attribution?.firstVisitedAt || null,
      p_landing_url: attribution?.landingUrl || null,
      p_referrer_url: attribution?.referrerUrl || null,
      p_attribution_source: attribution?.source || null,
      p_enquiry_page_url: enquiryPageUrl || null,
    })
    .single();
...
/**
 * "200-350k" → {min 200000, max 350000}; "500k-1m" → {500000, 1000000};
 * "1m+" → {1000000, null}; "under 400k" / "400k" → {null, 400000}.
 * The old parseBudget stripped every non-digit, so "200-350k" became 200,350
 * and "1m+" became 1 — hence the "€100 budgets" in the CRM (7 Sep 2026 audit).
 */
export function parseBudgetRange(budget) {
  if (!budget) return { min: null, max: null };
  const s = String(budget).toLowerCase().replace(/,/g, '').replace(/\s/g, '');
  ...
  const nums = matches.map((m) => toNum(m[1], m[2])).filter((n) => Number.isFinite(n) && n > 0);
  if (!nums.length) return { min: null, max: null };
  if (s.includes('+')) return { min: Math.min(...nums), max: null };
  if (s.startsWith('under') || s.startsWith('upto') || s.startsWith('<') || nums.length === 1) return { min: null, max: Math.max(...nums) };
