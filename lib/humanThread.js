...
import { gmailConnected, humanConversationWith } from '@/lib/gmail';

// enquiry_merged is NOT here: it is written on every gallery unlock by an
// existing contact, so it stood the follow-ups down for people who had only
// ever opened galleries (Barbara, Robert — 12 Sep). A real form submission
// always writes enquiry_submitted / gallery_enquiry / tour_request as well.
const CONVERSATION_ACTIVITIES = [
  'enquiry_submitted', 'gallery_enquiry', 'tour_request', 'email', 'phone_call', 'note',
...
export async function humanOwnsThread(db, { contactId, email }) {
  ...
  // Last word goes to the mailbox itself. The CRM sync copies Gmail into
  // activities hourly, but it started on 12 Sep 2026 and looks back only a
  // few hours; anything it missed, Gmail still has.
  if (addr && process.env.GMAIL_OAUTH_CLIENT_ID) {
    try {
      if (await gmailConnected(db)) {
        const why = await humanConversationWith(db, addr);
        if (why) return { owned: true, reason: `${why} (Gmail)` };
      }
    } catch (e) { console.warn('[humanThread] gmail check failed', e?.message || e); }
  }
