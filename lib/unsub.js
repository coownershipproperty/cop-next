...
/** Where List-Unsubscribe mailto clicks land (handled by hand, like today). */
const CONTACT_MAILTO = 'mailto:info@co-ownership-property.com?subject=unsubscribe';

/**
 * RFC 8058 one-click unsubscribe headers for a recipient. Pass the result as
 * `headers` to sendHtml() on EVERY subscriber-facing send.
 *
 * Why this exists: Gmail and Yahoo's bulk-sender rules require one-click
 * unsubscribe headers for anyone sending 5k+/day, and both providers surface
 * the header as the native "Unsubscribe" button next to the sender name —
 * which many recipients try FIRST, before ever scrolling to the footer.
 * Philip Harvey's "I have tried to unsubscribe a number of times" (23 Aug
 * 2026) is what an email without these headers looks like from the outside.
 *
 * The https target is /api/unsubscribe (not the /unsubscribe page): mailbox
 * providers POST the form body "List-Unsubscribe=One-Click" straight to this
 * URL with no JavaScript, so it must be the endpoint itself. The endpoint
 * 302-redirects GETs to the human page, so clients that treat it as a plain
 * link still work. Tokenless when no secret is configured — the endpoint
 * accepts an untokenised one-click POST precisely so those sends still get a
 * working button (see the note in pages/api/unsubscribe.js).
 */
export function listUnsubHeaders(email) {
  const clean = normalizeEmail(email);
  if (!clean) return { 'List-Unsubscribe': `<${CONTACT_MAILTO}>` };
  const token = unsubToken(clean);
  const url = `${BASE_URL}/api/unsubscribe?e=${encodeURIComponent(clean)}${token ? `&t=${token}` : ''}`;
  return {
    'List-Unsubscribe': `<${url}>, <${CONTACT_MAILTO}>`,
