 ...
 *
 * When the gallery-nurture and enquiry-check emails go out, and the rules
 * that stop them going out. The words themselves live elsewhere: the gallery
 * sequence (Day 1 / Day 4 / Day 10) is built in lib/galleryNurture.js and
 * designed in emails/gallery-nurture.tsx. This file owns timing and safety.
 *
...
import { buildNurtureStep, NURTURE_STEPS, NURTURE_DELAY_DAYS } from '@/lib/galleryNurture';
...
function footerHtml(email) {
  ...
  return `
<p style="margin:28px 0 0;font-family:Arial,sans-serif;font-size:11px;color:#6b6b6b;line-height:1.6;">
  You're receiving this because you requested photos or made an enquiry on
  <a href="${BASE_URL}" style="color:#6b6b6b;">co-ownership-property.com</a>.
...
<body style="margin:0;padding:0;background:#f5f5f5;">
  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f5f5f5;">
    <tr><td align="center" style="padding:28px 12px;">
      <table width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;width:100%;background:#ffffff;border:1px solid #e6e6e6;">
        <tr><td style="padding:24px 40px 16px;border-bottom:1px solid #e6e6e6;">
          <span style="font-family:'Poppins','Inter','Helvetica Neue',Helvetica,Arial,sans-serif;font-size:20px;letter-spacing:0.18em;color:#111111;">C<span style="color:#111111;">O</span>P</span>
          <span style="float:right;font-family:Arial,sans-serif;font-size:10px;letter-spacing:0.14em;color:#8a8a8a;text-transform:uppercase;padding-top:7px;">Co-Ownership Property</span>
        </td></tr>
...
function propLink(p) {
  ...
  return p.url
    ? `<a href="${escapeHtml(p.url)}" style="color:#111111;text-decoration:underline;">${title}</a>`
    : `<strong>${title}</strong>`;
}
 ...
 * table/tr/td with inline styles only, ink (#111111) header row with white
 * text, subtle alternating rows, champagne (#111111) accents, max 600px.
 * Whole-home figures use the property's share denominator (×8 for the
 * standard 1/8 share).
 *
 * NO concrete running-cost/annual-cost figures anywhere in this table
 * (Dylan, 18 Jul 2026): the estimates can't be stood behind. The running-
 * costs row is qualitative only — "1/8th of the home's total running costs,
...
function ownershipTableHtml(property) {
  ...
  const rows = [
    ...
    [
      ...
      'All of them — yours alone',
      `You pay <strong>1/${d}th</strong> of the home's total running costs, shared between all owners`
    ],
    [
  ...
  const trs = rows.map((cells, i) => {
    ...
    return `<tr style="background:${bg};">`
      + `<td style="padding:10px 12px;border-bottom:1px solid #e6e6e6;font-family:Arial,sans-serif;font-size:13px;color:${INK};font-weight:bold;">${cells[0]}</td>`
      + `<td style="padding:10px 12px;border-bottom:1px solid #e6e6e6;font-family:Arial,sans-serif;font-size:13px;color:#555555;">${cells[1]}</td>`
      + `<td style="padding:10px 12px;border-bottom:1px solid #e6e6e6;font-family:Arial,sans-serif;font-size:13px;color:${INK};">${cells[2]}</td>`
      + `</tr>`;
  }).join('');
...
// ── Templates ───────────────────────────────────────────────────────────────
//
// SUPERSEDED, 16 Sep 2026. Everything from here to buildEnquiryCheckD7 is the
// old plain-HTML D2/D5 nurture copy. The live gallery sequence is now Day 1 /
// Day 4 / Day 10, written in the newsletter design — see lib/galleryNurture.js
// and emails/gallery-nurture.tsx. Nothing calls the four builders below any
// more; they are kept only because the D5 ownership-comparison table
// (ownershipTableHtml) is the best short explanation of co-ownership vs a
// second home that we have written, and the property page is the obvious
// place for it. Do not wire these back into the sequence.
...
function tableShell(headers, rows) {
  ...
  const th = headers.map((h, i) =>
    `<th align="left" style="padding:10px 14px;background:${i === last ? '#f5f5f5' : '#f5f5f5'};border-bottom:1px solid #111111;font-family:Arial,sans-serif;font-size:10px;color:${i === last ? '#111111' : '#6b6b6b'};letter-spacing:0.1em;text-transform:uppercase;">${h}</th>`
  ).join('');
  const trs = rows.map(cells =>
    `<tr>${cells.map((c, i) => `<td style="padding:11px 14px;border-bottom:1px solid #e6e6e6;font-size:14px;line-height:1.55;${i === cells.length - 1 ? 'background:#f5f5f5;font-weight:bold;color:#111111;' : ''}${i === 0 ? 'color:#666666;' : ''}">${c}</td>`).join('')}</tr>`
  ).join('');
  return `
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:4px 0 22px;border-collapse:collapse;border:1px solid #e6e6e6;">
      <tr>${th}</tr>${trs}
    </table>`;
}

/** B-D2 — Day 2, multiple unlocks: the shortlist side by side.
 *  NO per-home cost comparison (Dylan, 18 Jul 2026 — explicitly rejected):
 *  the table carries price/beds/nights only, never estimated costs. */
export function buildNurtureD2Multi({ firstName, properties }) {
  ...
  const body = `
    ...
    ${tableShell(['Home', shareHeader(properties), 'Beds', 'Nights/yr'], rows)}
...
/**
 * Gallery nurture — still OFF. The two flags below are the whole switch.
 *
 * What went wrong the first time: the D2/D5 emails were queued with
 * `approved_at = NULL` against an engine that only sends approved rows, so
 * from July onwards the sequence quietly produced about ten rows a day that
 * could never send. 998 of them were cancelled on 4 Sep 2026.
 *
 * Rebuilt 16 Sep 2026 as three steps in the 9 Sep newsletter design
 * (emails/gallery-nurture.tsx; the copy and the fact plumbing live in
 * lib/galleryNurture.js and lib/nurtureFacts.js):
 *
 *   Day 1    the verified numbers for the home or homes they unlocked
 *   Day 4    the two closest homes we genuinely have
 *   Day 10   the offer of an introduction — which is the point of all three
 *
 * Before GALLERY_NURTURE_ENABLED goes true, two things have to be settled:
 *   1. David has seen the previews and signed off the copy.
 *   2. The approval rule is decided. Leaving AUTO_APPROVE false means every
 *      row waits for a human to approve it — which is exactly the state that
 *      built the pile last time, and is only right if someone is actually
 *      clearing that queue. Setting it true sends them on schedule.
 * Turning on (1) without answering (2) recreates the pile.
 */
const GALLERY_NURTURE_ENABLED      = false;
const GALLERY_NURTURE_AUTO_APPROVE = false;

// ── Scheduling: a gallery unlock queues Day 1, Day 4 and Day 10 ────────────

/** Every slug this person has unlocked, newest first — the pending rows
 *  remember the earlier ones, so a second unlock widens the sequence rather
 *  than starting a second one. */
function slugsFromPending(pending, newSlug) {
  const seen = new Set();
  const out = [];
  const push = (slug) => { if (slug && !seen.has(slug)) { seen.add(slug); out.push(slug); } };
  push(newSlug);
  for (const row of pending || []) {
    for (const s of (row.template_props && row.template_props.slugs) || []) push(s);
    for (const p of (row.template_props && row.template_props.properties) || []) push(p.slug);
  }
  return out;
}
 ...
 *
 *   - No pending rows  → queue Day 1, Day 4 and Day 10.
 *   - Pending rows and a new property → rebuild those rows around the wider
 *     set of homes, keeping each one's original send_after. One sequence per
 *     person, however many galleries they open.
 */
export async function scheduleGalleryNurture({ contactId, email, firstName, locale, propertySlug, propertyTitle, propertyUrl }) {
  try {
    if (!GALLERY_NURTURE_ENABLED) return;
    ...
    // Hidden and sold rows never enter the sequence (19 Jul incident, and
    // nothing is more embarrassing than pricing a house that has gone).
    ...
    const slugs = slugsFromPending(pending, propertySlug);

    // ── Widen the sequence already in flight ─────────────────────────────
    if (pending && pending.length) {
      const known = slugsFromPending(pending, null);
      if (known.length === slugs.length) return;  // nothing new — leave it alone
      for (const row of pending) {
        const step = String(row.template_name || '').replace(/^nurture_/, '');
        if (!NURTURE_STEPS.includes(step)) continue;
        const built = await buildNurtureStep(db, { step, slugs, firstName, email: cleanEmail });
        if (!built) continue;
        const { error } = await db.from('email_queue').update({
          subject:        built.subject,
          html:           built.html,
          template_props: { step, slugs, from: DYLAN_FROM, replyTo: DYLAN_REPLY },
          notes:          `Gallery nurture ${step} (rebuilt — another gallery opened)`,
        }).eq('id', row.id);
        if (error) console.error('[followupSequence] nurture rebuild failed:', error.message);
      }
      return;
    }

    // ── Fresh sequence — but never restart one inside the cooldown ────────
    ...
    const now = Date.now();
    const approvedAt = GALLERY_NURTURE_AUTO_APPROVE ? new Date().toISOString() : null;
    const rows = [];
    for (const step of NURTURE_STEPS) {
      const built = await buildNurtureStep(db, { step, slugs, firstName, email: cleanEmail });
      if (!built) continue;   // e.g. Day 4 with nothing worth offering
      rows.push({
        ...queueRow({
          email: cleanEmail, firstName, contactId,
          sequenceType: SEQ_GALLERY, step: `nurture_${step}`,
          subject: built.subject, html: built.html,
          sendAfter: now + NURTURE_DELAY_DAYS[step] * DAY_MS,
          props: { slugs },
        }),
        approved_at: approvedAt,
        notes: `Gallery nurture ${step}`,
      });
    }
...
export async function preflightSequenceEmail(db, row, nowMs = Date.now()) {
  ...
  // Suppression list. For sequences and marketing it always wins. For a
  // one-to-one reply Dylan approved on /admin/replies, only a bounced or
  // complained address blocks the send — someone who unsubscribed from the
  // newsletter months ago and now writes in with a question must still get
  // an answer (7 Sep 2026 audit).
  const isReplyDraft = ['enquiry_reply', 'enquiry_reply_draft'].includes(row.trigger);
  if (isReplyDraft) {
    const { data: hard } = await db.from('suppressions').select('reason')
      .ilike('email', email).in('reason', ['bounced', 'complained']).limit(1);
    if (hard && hard.length) {
      return { action: 'cancel', notes: `Cancelled — address is ${hard[0].reason} (hard suppression)` };
    }
  } else if (await isSuppressed(db, email)) {
