...
/**
 * Gallery nurture: OFF since 4 Sep 2026.
 *
 * These D2/D5 emails were queued with `status = 'pending'` and `approved_at =
 * NULL`, and the email engine only sends approved rows — so from July onwards
 * the sequence quietly generated ~10 rows a day that could never send. 998 of
 * them were cancelled on 4 Sep; this flag stops new ones being created.
 *
 * Nothing below is deleted, because the sequence itself is a good idea — it
 * just never had an approval rule or reviewed copy. To bring it back:
 *   1. Write the copy in the Template Studio (/admin/templates) so it can be
 *      read and changed without a deploy, as the other lead emails now are.
 *   2. Decide the approval rule — auto-approve, or a queue Dylan clears.
 *   3. Set this to true.
 * Turning it back on without (1) and (2) just recreates the pile.
 */
...
export async function scheduleGalleryNurture({ contactId, email, firstName, locale, propertySlug, propertyTitle, propertyUrl }) {
  try {
    ...
    // Hidden/staged rows never enter the nurture sequence (19 Jul incident).
