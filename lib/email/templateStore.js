...
export function renderTemplate(row, data = {}) {
  ...
  return {
    subject,
    html: shell(body + signatureHtml(design), row.locale || data.locale, design, pre),
  ...
  for (const loc of tryLocales) {
    ...
    try {
      ...
      const { data, error } = await db
        ...
        .select('id,moment,locale,channel,version,tier,subject,preheader,blocks,design,from_name,from_email,reply_to,active')
...
export async function buildEmail(moment, locale, data, fallback) {
  try {
    ...
    if (row) {
      ...
      if (out.subject && out.html) return { ...out, source: 'studio', version: row.version };
    }
  } catch (e) {
