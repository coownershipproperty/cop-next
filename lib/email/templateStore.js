...
export const DEFAULT_DESIGN = {
  bodyFont:    BRAND.sans,
  headingFont: BRAND.serifTight,
  fontSize:    '15px',
  lineHeight:  '1.7',
  textColor:   BRAND.ink,      // note-style body copy — see brand.js
  headingColor:BRAND.navy,
  linkColor:   BRAND.navy,
  accentColor: BRAND.gold,
  background:  BRAND.paper,
  padding:     BRAND.shellPadding,
  maxWidth:    null,
  signature:   true,
  ...
  paragraphGap:  BRAND.paragraphGap,
  signatureFont: BRAND.sansTight,
  showUnsubscribe:  true,
  footerNote:       'You received this because you asked for photos on co-ownership-property.com.',
  unsubscribeLabel: 'Unsubscribe',

  /**
   * `bare` — for an actual reply Dylan sends from Gmail.
   *
   * No wrapper, no fonts, no colours, no padding, no signature block, no
   * unsubscribe: just paragraphs. Gmail then renders it in its own default,
   * which is exactly what a message looks like when a person types one.
   *
   * This matters more than it sounds. A designed HTML email inside a reply
   * thread is a tell — padding, a gold button and a rendered signature say
   * "a machine wrote this", which destroys the trust the reply exists to
   * build. For a personal reply the plainness IS the design.
   */
  bare: false,
};
...
export function interpolate(str, data) {
  ...
  out = out.replace(/\{\{(&?)\s*([\w.]+)\s*(?:\|([^}]*))?\}\}/g, (_, raw, key, fallback) => {
    ...
    // Escape HTML, then neutralise the three markdown characters too: the
    // markdown pass runs after interpolation, so a lead-supplied first name of
    // "[Verify here](https://evil.example)" used to become a real link in an
    // email signed by Dylan (7 Sep 2026 audit). Entities render identically.
    return escapeHtml(v).replace(/\[/g, '&#91;').replace(/\]/g, '&#93;').replace(/\*/g, '&#42;');
  });
...
function renderBareBlock(block, data) {
  const type = String(block && block.type || 'text');
  const md = (v) => inlineMarkdown(interpolate(v, data)).replace(/ style="[^"]*"/g, '');
  switch (type) {
    case 'text':
    case 'heading': {
    ...
    case 'list': {
      const lis = (block.items || []).map(md).filter(x => x.trim()).map(x => `<li>${x}</li>`).join('');
      return lis ? `<ul>${lis}</ul>` : '';
    }
    case 'button': {
      const url = safeUrl(interpolate(block.url, data));
      const label = interpolate(block.label, data);
      return url && label ? `<p><a href="${escapeAttr(url)}">${label}</a></p>` : '';
    }
    case 'slot': {
      const v = lookup(data, block.slot);
      // Generated blocks are styled markup — strip it so a reply stays plain.
      return v ? String(v).replace(/ style="[^"]*"/g, '') : '';
    }
    case 'html': return interpolate(block.html, data);
...
<table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-top:26px;padding-top:14px;border-top:1px solid #ededed;">
  <tr><td style="font-family:${design.signatureFont};font-size:11px;line-height:1.6;color:#6b6b6b;">
    ${note}<a href="{{UNSUB_URL}}" style="color:#6b6b6b;text-decoration:underline;">${escapeHtml(design.unsubscribeLabel || 'Unsubscribe')}</a>.
  </td></tr>
</table>`;
}
...
export function renderTemplate(row, data = {}) {
  ...
  // A personal reply is not a designed email. No shell, no signature block
  // (Gmail appends Dylan's own), no unsubscribe (this is one-to-one mail).
  if (design.bare) {
    const bare = blocks.map(b => renderBareBlock(b, data)).filter(Boolean).join('\n');
    return {
      subject,
      html: bare,
