...
async function runJourney({ db, now, dryRun, testEmails, journey, results, summary }) {
  ...
  for (const [contactId, acts] of byContact) {
    ...
    if (!journey.transactional) {
      ...
      // A template never writes to someone a person already has (11 Sep 2026
      // — see lib/humanThread.js). Marker written so they are not retried.
      const owned = await humanOwnsThread(db, { contactId, email });
      if (owned.owned) {
        if (!dryRun) await writeMarker(db, journey, contact, 'cancelled',
          { notes: `stood down: ${owned.reason}` });
        summary.humanOwned = (summary.humanOwned || 0) + 1;
        results.push({ journey: journey.id, email: contact.email,
          decision: dryRun ? 'would_skip_human_thread' : 'skipped_human_thread', reason: owned.reason });
        continue;
      }
