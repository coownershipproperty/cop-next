...
function galleryLinksHtml(properties, t) {
  ...
  const items = withLinks.map(p =>
    `<li style="margin:0 0 6px;">${escapeHtml(friendlyName(p.title, t).replace(/^./, c => c.toUpperCase()))} — <a href="${escapeHtml(p.galleryUrl)}" style="color:#111111;text-decoration:underline;">${t.galleryLinkLabel}</a></li>`
  ).join('');
  return `
