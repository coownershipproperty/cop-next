...
import { localeColumns, localizedField, numberLocale, routePath } from '@/lib/i18n';
...
export function localeHomeStaticProps(locale) {
  return async function getStaticProps() {
    ...
    // Blog posts are English-only today. For es/fr/de that is a reasonable
    // fallback: those locales route /xx/blog/, so the card keeps the reader
    // inside their own language even though the article is in English.
    //
    // it/nl/pt have no blog route at all (ROUTE_SLUGS marks blog null), so
    // their cards linked to /blog/ — three English headlines that drop an
    // Italian reader out of the Italian site with no way back but the back
    // button. An absent section is better than an exit. (16 Sep 2026)
    const hasBlog = Boolean(routePath(locale, 'blog'));
    const { data: postRows } = hasBlog
      ? await supabase
          .from('posts')
