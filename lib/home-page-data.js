...
/**
 * getStaticProps for the how-it-works page of `locale`: the copy file plus the
 * live inventory strip in the hero.
 *
 *   export const getStaticProps = howItWorksStaticProps('es');
 */
export function howItWorksStaticProps(locale) {
  return async function getStaticProps() {
    const copy = JSON.parse(
      fs.readFileSync(path.join(process.cwd(), 'content', 'how', `${locale}.json`), 'utf-8')
    );
    let inventory = null;
    try {
      const { getInventory } = await import('@/lib/inventory');
      const inv = await getInventory();
      const eurMins = Object.values(inv.countries)
        .filter(c => c.available > 0 && c.min && c.minCurrency === 'EUR')
        .map(c => c.min);
    ...
    return { props: { locale, copy, inventory }, revalidate: 3600 };
  };
}
