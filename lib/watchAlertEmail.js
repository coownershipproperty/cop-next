...
export function shell(bodyHtml, email) {
  ...
  <div style="background:#f5f5f5;padding:40px 16px;font-family:'Poppins','Inter','Helvetica Neue',Helvetica,Arial,sans-serif;color:#111111">
    <div style="max-width:520px;margin:0 auto;background:#ffffff;border:1px solid #e6e6e6">
      <div style="padding:30px 32px 24px;text-align:center;border-bottom:1px solid #e6e6e6">
        <a href="${SITE}"><img src="${SITE}/images/email-logo-dark.png" width="132" height="65" alt="COP — Co-Ownership Properties" style="display:inline-block;width:132px;height:auto;border:0"/></a>
      </div>
...
export function propCard(p, locale = 'en', perLabel = 'per share') {
  ...
  return `
  <div style="border:1px solid #e6e6e6;margin:0 0 14px">
    ${p.img ? `<a href="${href}"><img src="${p.img}" width="100%" style="display:block;max-height:220px;object-fit:cover" alt=""/></a>` : ''}
    <div style="padding:14px 16px">
      <p style="font-size:15px;margin:0 0 6px"><a href="${href}" style="color:#111111;text-decoration:none"><strong>${p.title}</strong></a></p>
      <p style="font-family:Arial,sans-serif;font-size:12px;color:#6b6b6b;margin:0">${p.price ? `${fmt(p.price, p.currency)} ${perLabel}` : ''}${p.beds ? ` · ${p.beds} beds` : ''}</p>
    </div>
  </div>`;
}
...
export function askBlock(t, href) {
  ...
  <p style="margin:0 0 26px">
    <a href="${href}" style="display:inline-block;background:#111111;color:#ffffff;text-decoration:none;font-family:Arial,sans-serif;font-size:13px;letter-spacing:0.08em;padding:12px 22px">${t.cta}</a>
  </p>
  <p style="font-size:15px;line-height:1.7;margin:0">${t.sign}<br/>
    <span style="font-family:Arial,sans-serif;font-size:12px;color:#6b6b6b">${t.role}</span></p>`;
}
...
export function buildWatchEmail({ property, email, locale = 'en', kind, oldPrice }) {
  ...
  const body =
    `<p style="font-family:Arial,sans-serif;font-size:12px;color:#6b6b6b;margin:0 0 14px">${t.intro}</p>` +
