...
const COLLECTION_COLS = 'slug, name, tagline, description, share_type, status, price, currency, price_note, monthly_cost, homes_count, weeks_per_year, availability_note, highlights, faqs, hero_image, sort, destination_photos';
const HOME_COLS = 'collection_slug, position, key, chapter, name, city, region, country, home_type, size_m2, plot_m2, bedrooms, bathrooms, guest_toilets, extra_beds, readiness, description, features, airport_note, photos, photos_note, example_photos, example_note, floorplans, search_terms';
...
export function collectionCardData(c) {
  return {
    ...
    homes: (c.homes || []).map(h => {
      // Public card photo: the home itself only when it is ready; otherwise a
      // destination view of the place (never a past home, David 24 Sep 2026).
      const ready = h.readiness === 'ready' && (h.photos || []).length > 0;
      const dest = (c.destination_photos || []).find(d => d.key === h.key);
      return {
        key: h.key, chapter: h.chapter || null, name: h.name, city: h.city, region: h.region, country: h.country,
        photo: ready ? h.photos[0] : (dest?.src || null), photoIsDestination: !ready && !!dest,
        search_terms: h.search_terms || [], readiness: h.readiness,
        // The Our Homes filters speak the listings' vocabulary (country
        // "England", region "Paris"), not the collection page's ("United
        // Kingdom", "Le Marais"). These two fields are only for matching.
        fCountry: filterCountry(h), fRegion: filterRegion(h),
      };
    }),
  };
}

const COUNTRY_ALIASES = { 'United Kingdom': 'England', UK: 'England' };
const CITY_REGIONS = new Set(['London', 'Paris', 'Rome']);
function filterCountry(h) { return COUNTRY_ALIASES[h.country] || h.country; }
function filterRegion(h) {
  if (CITY_REGIONS.has(h.city)) return h.city;
  if (h.region) return h.region;
  if (h.country === 'France' && /south of france|provence/i.test(h.city || '')) return "Côte d'Azur";
  return h.city || null;
}
...
// "∼12" — David, 24 Sep 2026: the weeks figure is an average, so it always
// carries the circa wave, and in running text reads "circa 12 weeks".
export function weeksLabel(c) {
  return `\u223C${Math.round(c?.weeks_per_year || 12)}`;
}
