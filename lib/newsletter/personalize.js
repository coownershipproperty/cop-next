...
/**
 * Newsletter prices are shown rounded — "$99,000", not "$99,038". Exact figures
 * read like a spreadsheet in a marketing email; the listing page has the exact
 * price. Under 1m: nearest 1,000. From 1m: nearest 10,000.
 */
export function formatPriceRounded(price, currency) {
  const n = Number(price || 0);
  const step = n >= 1_000_000 ? 10_000 : 1_000;
  return formatPrice(Math.round(n / step) * step, currency);
}
