...
/**
 * A contact's place interests, most recent first — every place they have
 * looked at, not only what they typed into an enquiry form: their leads,
 * floor-plan and gallery requests, watches, discreet unlocks, tour requests,
 * the gallery emails we sent them and their saved searches. Read from the
 * v_contact_region_signals view (migration 14 Sep 2026).
 *
 * Returns Map<contactId, { interests: [{mainRegion, subregion}], budgetMax }>.
 * `interests` is deduped and ordered by the most recent signal for that
 * place, so the lead home is the place they looked at last.
 */
export async function prefetchInterests(db, contactIds) {
  const byContact = new Map();
  const entryFor = (id) => {
    let e = byContact.get(id);
    if (!e) { e = { interests: [], budgetMax: null, _seen: new Set() }; byContact.set(id, e); }
    return e;
  };
  const ids = [...new Set((contactIds || []).filter(Boolean))];
  for (const slice of chunk(ids, 200)) {
    const { data: rows, error } = await db
      .from('v_contact_region_signals')
      .select('contact_id, region, city, at')
      .in('contact_id', slice)
      .order('at', { ascending: false })
      .limit(5000);
    if (error) throw new Error('interest prefetch failed: ' + error.message);
    for (const r of rows || []) {
      const main = (r.region || '').trim() || null;
      const sub  = (r.city   || '').trim() || null;
      if (!main && !sub) continue;
      const e = entryFor(r.contact_id);
      const key = `${main}|${sub}`;
      if (e._seen.has(key)) continue;
      e._seen.add(key);
      e.interests.push({ mainRegion: main, subregion: sub });
    }
    const { data: leads, error: leadErr } = await db
      .from('leads')
      .select('contact_id, budget_max')
      .in('contact_id', slice)
      .not('budget_max', 'is', null);
    if (leadErr) throw new Error('budget prefetch failed: ' + leadErr.message);
    for (const l of leads || []) {
      const e = entryFor(l.contact_id);
      const b = Number(l.budget_max);
      if (b && (!e.budgetMax || b > e.budgetMax)) e.budgetMax = b;
    }
  }
  for (const e of byContact.values()) delete e._seen;
  return byContact;
}

/**
 * Fetch one contact's interests (same signals as prefetchInterests).
 * Used for per-recipient property reordering in previews and the drain.
 */
export async function getContactInterests(db, contactId) {
  const map = await prefetchInterests(db, [contactId]);
  return map.get(contactId) || { interests: [], budgetMax: null };
}
