...
import { formatPriceRounded as formatPrice } from '@/lib/newsletter/personalize';
...
const TEMPLATES = {
  ...
  // "Last shares" (24 Sep 2026): one home close to selling out, sent only to
  // people who once asked about it. Same design, its own words below; the
  // campaign's intro_text carries the story.
  'last-shares':             PersonalisedNewsletter,
...
const ANNOUNCEMENTS = {
  'last-shares': {
    eyebrow:     'Last shares',
    // Villa Soie wording (24 Sep 2026). Change it for the next home.
    headline:    () => 'Only two shares left',
    leadCta:     'See the home again',
    rowCta:      'Discover this home',
    buttonLabel: () => 'View every home',
    buttonHref:  `${base}/our-homes/`,
    nudge:       'Would you like to talk it through? Just reply to this email.',
  },
  'discreet-release': {
 ...
 */
export function applyMergeTags(str, { firstName, region, count, topRegion, topPlace, topRegions } = {}) {
  if (!str) return '';
  const values = {
    ...
    // {{top_regions}}: the reader's matched places first, topped up with the
    // best-represented regions in the email to three, then "and more" —
    // "Mallorca, Côte d'Azur, Lake Garda and more". (David, 15 Sep 2026:
    // one place alone reads as if that is all there is.)
...
export async function renderRecipient({
  ...
  // Reader's matched regions in order, then the regions with the most homes
  // in this email, up to three names; "and more" when the email holds others.
  const matchedRegions = [...new Set(primary.map(p => p.regionTag).filter(Boolean))];
  const regionCounts = {};
  for (const p of allProps) if (p.regionTag) regionCounts[p.regionTag] = (regionCounts[p.regionTag] || 0) + 1;
  const popular = Object.entries(regionCounts).sort((a, b) => b[1] - a[1]).map(([r]) => r);
  const named = [...matchedRegions, ...popular.filter(r => !matchedRegions.includes(r))].slice(0, 3);
  const topRegionsStr = named.length
    ? (Object.keys(regionCounts).length > named.length
        ? named.join(', ') + ' and more'
        : named.length > 1 ? named.slice(0, -1).join(', ') + ' and ' + named[named.length - 1] : named[0])
    : '';
  const tags = { firstName, region: regionStr, count: displayCount, topRegion, topPlace, topRegions: topRegionsStr };
