 ...
 */
export function applyMergeTags(str, { firstName, region }) {
  if (!str) return '';
  return String(str)
...
export async function renderRecipient({
  ...
  const finalSubject = subjectTemplate
    ? applyMergeTags(subjectTemplate, { firstName, region: regionStr })
    : (firstName !== 'there'
  ...
  const finalIntro = introText
    ? applyMergeTags(introText, { firstName, region: regionStr })
    : null;
  ...
  if (templateType === 'new-listings-digest') {
    ...
    // We hide past dated viewings here (same logic as the /viewings/ page) and
    // resolve each card's hero image from the live property record so the
    // newsletter always shows whatever's currently on the property page.
    const today = new Date().toISOString().slice(0, 10);
    const propsJson = (await import('@/lib/properties.json')).default;
    const bySlug = Object.fromEntries(propsJson.map(p => [p.slug, p]));
