/**
 * pages/fr/partenaires/[slug].js — French partner profile pages
 * URL pattern: /fr/partenaires/{slug}/
 */
...
const LOCALE = 'fr';
const META_FILE = 'partners-meta-fr.json';
const CONTENT_DIR = 'content/partners/fr';
...
export async function getStaticProps({ params }) {
  ...
  body = fillInventoryDeep(injectH2Ids(body), inv, { locale: 'fr' });
  const filledEntry = fillInventoryDeep(entry, inv, { locale: 'fr' });
  const inventory = inventorySummary(inv, { partner: slug, locale: 'fr' });
...
export default function FrPartnerPage(props) {
