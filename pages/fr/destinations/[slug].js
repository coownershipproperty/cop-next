...
import { destinationAvailableIn, localeColumns, pickLocalized, localizedField } from '@/lib/i18n';
...
export async function getStaticProps({ params }) {
  ...
  const { data: allProps } = await supabase
    .from('properties')
    .select(`slug, ${localeColumns(['title'], { locales: ['fr'] })}, img, images, total_images, drive_url, price, currency, share_denominator, country, region, city, beds, size, status, property_type, is_discreet`)
  ...
  const matchedProps = matchedRaw.slice(0, 60).map(p => ({
    ...
    ...pickLocalized(p, ['title'], { locales: ['fr'] }),
    img: p.img,
