/**
 * pages/es/socios/[slug].js — Spanish partner profile pages
 * URL pattern: /es/socios/{slug}/
 */
...
const LOCALE = 'es';
const META_FILE = 'partners-meta-es.json';
const CONTENT_DIR = 'content/partners/es';
...
export async function getStaticProps({ params }) {
  ...
  body = fillInventoryDeep(injectH2Ids(body), inv, { locale: 'es' });
  const filledEntry = fillInventoryDeep(entry, inv, { locale: 'es' });
  const inventory = inventorySummary(inv, { partner: slug, locale: 'es' });
...
export default function EsPartnerPage(props) {
