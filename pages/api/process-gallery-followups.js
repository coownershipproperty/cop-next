...
// Gallery follow-up copy lives in lib/email/templates.js — this file used to
// carry a byte-identical duplicate of it.
import { GALLERY_FOLLOWUP_COPY as COPY } from '@/lib/email/templates';
...
function galleryLinksHtml(properties, t) {
  ...
  const items = withLinks.map(p =>
    `<li style="margin:0 0 6px;">${escapeHtml(capitalize(friendlyName(p.title, t)))} — <a href="${escapeHtml(p.galleryUrl)}" style="color:#111111;text-decoration:underline;">${t.galleryLinkLabel}</a></li>`
  ).join('');
  return `
...
async function buildForContact({ firstName, properties, locale, comparisonRows = [] }) {
  ...
  const linkParts = properties.map(p => {
    ...
    return p.url
      ? `<a href="${escapeHtml(p.url)}" style="color:#111111;text-decoration:underline;">${label}</a>`
      : `<strong>${label}</strong>`;
  });
